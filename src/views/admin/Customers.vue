    ...
    <div v-else>
      <div>
        <div class="d-sm-flex justify-space-between">
          <div>
            <div class="icon-bar ">
              <div class="ml-4 cursor-pointer" v-if="isTrash" @click="removeUsers(true)">
                <v-img
                  :src="restore"
                  width="20"
                  class="mr-2"
          ...
          <div class="d-sm-flex justify-space-between">
            ...
            <div class="search-bar header-bar">
              ...
              <input
                ...
                v-on:keyup.enter="filterCustomer"
              />
            </div>
          </div>
        </div>
        <div class="mt-4">
          <div class="title-table pa-1 pa-md-4 list-order-tabs">
            <div class="icon-bar tab-left" @click="changeContentTabs('order-list', false)" v-bind:class="{active: contentTab === 'order-list'}">
            ...
            <div class="d-flex icon-bar tab-right" @click="changeContentTabs('trash', true)" v-bind:class="{active: contentTab === 'trash'}">
              <button
                :disabled="checkTrash"
                class="check-trash" style="outline: none;"
                :class="{
          ...
          <div class="list-order list" v-else>
            <div v-if="filter" class="text-center py-3">
              <v-progress-circular
                indeterminate
                color="primary"
              ></v-progress-circular>
...
<script>
import { getCustomers, deActiveUsers, countCustomerIsTrash } from "@store/user";
...
export default {
  ...
  methods: {
    async getCustomers() {
      ...
      if(this.searchCustomer) query.search = this.searchCustomer
    ...
    async countTrash(){
      const result = await countCustomerIsTrash()
      if (result.statusCode === 200){
